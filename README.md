...
There is currently no built-in mechanism that helps users measure or optimize the environmental impact of their AI usage.
...
🚀 Key Features
1. One-Click Prompt Optimization
...
Energy usage is estimated based on token count and an assumed energy-per-token constant.
Savings are converted to watt-hours (Wh).
